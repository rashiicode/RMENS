...
const home = async (req, res) => {
  if (req.session.user) {
    ...
    res.render("user/home",{categoryList:categoryList});
  }
};
...
const profile = async (req, res) => {
  try {
    // Assuming userCollection.find() returns a single user object
    const profilemain = await userCollection.findOne({ email:req.session.user });

    const addressexist=await addressCollection.findOne({userId:profilemain._id});
    ...
    res.render("user/profile", { profilemain ,addressexist}); // Pass profilemain as an object
  } catch (error) {
    // Handle any errors
...
const addProfile = async (req, res) => {
  try {
    ...
    const profile = await userCollection.findOne({ email: req.session.user });
    console.log("whcjh",profile);

    // Render the view with the populated profile
    res.render("user/addNewDetail", { profile });
  } catch (error) {
...
const addProfilePost = async (req, res) => {
  try {
    const profileId = req.params.id;

    const user = await addressCollection.findOne({userId:profileId});
    ...
    if (!user) {
      ...
      await addressCollection.create({
        userId: profileId, // assuming profileId is the user's ID
        address: [address] // creating a new user with the provided address
      });
    } else {
      // If the user exists, update their existing address
      const { street, city, state, zipcode, phonenumber } = req.body;
      const address = {
        street,
        city,
    ...
    console.log("RASHIIIII CHQ OF ADDRESS ADD",user);

    res.redirect("/profile");

  } catch (error) {
    console.log("Error occurred: ", error.message);
    res.status(500).send("An error occurred while saving profile data.");
  }
};
...
const editProfile = async (req, res) => {
  try {
    ...
    const addressEdit = await addressCollection.findOne(
      { },
      { address: { $elemMatch: { _id:addressId } } }
  );
  
    console.log("addressEdit",addressEdit);
    res.render("user/editProfile", { addressEdit });
  } catch (error) {
...
const editProfilePost = async (req, res) => {
  try {
    ...
    const updatedAddress = await addressCollection.findByIdAndUpdate(
      addressId, // Use the extracted address ID to identify the address
      {
        $set: {
          ...
          "address.0.phonenumber": phonenumber
        }
      },
      { new: true }
    );
...
const deleteProfile = async (req, res) => {
  try {
    console.log("Entered deleteAddress controller");
    const removingAddressId = req.params.addressId; // Assuming correct conversion logic
    // console.log("Removing address from profile Id:", removingProfileId);
    console.log("Removing address Id:", removingAddressId);
    
  
    const result = await addressCollection.findByIdAndDelete(
      removingAddressId,
      { $pull: { "address": { _id: removingAddressId } } },
      { new: true }
    );
    ...
    if (result) {
      ...
      console.log("Profile not found or address not removed");
      res.status(404).send("Profile not found or address not removed");
    }
  } catch (error) {
...
const emailResendOtp = async (req, res) => {
  try {
    ...
    if (currentTime - lastOtpRequestTime < 60000) {
      return res.render("user/signupOtp", {
        message: "Wait for 60 seconds before requesting a new OTP",
      });
    }
